...
Project ordering defaults to manual. `session-display-mode` persistence is version 5: legacy `recent` project ordering migrates to `manual`, the removed `displayMode` field is dropped, and `mobileLandingMode` defaults to `last-session` when absent. `recents` is therefore an explicit opt-in for phones with no open session or draft.
